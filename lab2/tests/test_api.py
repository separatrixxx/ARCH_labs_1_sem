...
async def register(client, login, first_name="Nikita", last_name="Lokhmatov", password="password123"):
...
async def test_find_user_by_name_mask(service_client):
    ...
    await register(service_client, "mask_user2", first_name="Nikita", last_name="Lokhmatov")

    resp = await service_client.get(
        "/api/v1/users",
        params={"last_name": "Lokhmatov"},
        headers={"Authorization": f"Bearer {token}"},
    )

    assert resp.status == 200
    assert any(u["last_name"] == "Lokhmatov" for u in resp.json())
